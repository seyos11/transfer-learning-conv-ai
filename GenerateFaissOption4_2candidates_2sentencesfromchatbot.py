            ...
            for _ in range(args.personality_permutations):
                for utterance in dialog["utterances"]:
                    ...
                    for j, candidate in enumerate(utterance["candidates"][-num_candidates:]):
                        ...
                        if len(history) >1:
                            if len(history)  > 3:
                                history_encoded_chatbot = model.encode([history[-2],history[-4]], show_progress_bar=False)
                            else:
...
def train():
    ...
    with open('data_persona_faiss_fase1_opcion4_two_sentences_from_history.pkl', 'wb') as f:
        pickle.dump(persona_faiss, f)
    with open('data_persona_faiss_index_fase1_opcion4_two_sentences_from_history.pkl', 'wb') as f:
