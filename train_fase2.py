...
from utils_2 import get_dataset, make_logdir
...
def build_input_from_segments(persona1, persona2, history, reply, tokenizer, lm_labels=False, with_eos=True):
    """ Build a sequence of input from 3 segments: persona, history and last reply """
    bos, eos, persona1_token,persona2_token,speaker1, speaker2 = tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS_2[:-1])

    instance = {}
    sequence = [[bos] + [persona1_token]+ list(chain(*persona1))]+ [[persona2_token] + list(chain(*persona2))] + history + [reply + ([eos] if with_eos else [])]
    #sequence = [sequence[0]] + [[speaker2 if (len(sequence)-i) % 2 else speaker1] + s for i, s in enumerate(sequence[1:])]
    #Dialog_State
    sequence = [sequence[0]] + [sequence[1]] + [[speaker2 if (len(sequence)-i) % 2 else speaker1] + s for i, s in enumerate(sequence[2:])]
            ...
            for _ in range(args.personality_permutations):
                for utterance in dialog["utterances"]:
                    ...
                    for j, candidate in enumerate(utterance["candidates"][-num_candidates:]):
                        ...
                        instance, _ = build_input_from_segments(persona1, persona2, history, candidate, tokenizer, lm_labels)
