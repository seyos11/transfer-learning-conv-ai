...
import faiss
from train import pad_dataset, SPECIAL_TOKENS, add_special_tokens_, build_input_from_segments
...
class TransformerAgent(Agent):
    ...
    def observe(self, observation):
        ...
        history_encoded_user = self.model.encode([self.history_not_tokenized[-1]],show_progress_bar=False)
        D, I = index.search(np.array(history_encoded_user), k=len(self.persona_not_tokenized))
        #history_faiss_selected.append(history)
        
        
        index_to_be_removed = I[0][0]

        persona2_not_tokenized = self.persona_not_tokenized[:index_to_be_removed] + self.persona_not_tokenized[index_to_be_removed+1:]
        
        
        embeddings_persona2 = self.model.encode(persona2_not_tokenized, show_progress_bar=False)   
        # Step 1: Change data type
        ...
        if len(self.history_not_tokenized) >1:
            history_encoded_chatbot = self.model.encode([self.history_not_tokenized[-2]], show_progress_bar=False)
        else:
            history_encoded_chatbot = self.model.encode([self.history_not_tokenized[-1]], show_progress_bar=False)
        T, J = index2.search(np.array(history_encoded_chatbot), k=len(persona2_not_tokenized))
        self.persona_faiss_selected = persona2_not_tokenized[J[0][0]]
...
if __name__ == '__main__':
    ...
    parser.set_params(
        model='convai_evaluation_faiss_option4:TransformerAgent')
