...
def run():
    ...
    parser.add_argument("--n_sentences", type=int, default= 3, help="sentences used to get faiss personality")
    ...
    def postprocess_rouge (preds, labels):
 ...
 ''''''     dataset = get_data_loaders()        
    ...
    print(metric1x1)
    print(metric2x2) '''''' def compute_metrics(eval_pred):
    ...
    dataset4x4 = get_data_loaders()
    dataset3x3 = get_data_loaders_3sentences()
    dataset2x2 = get_data_loaders_2sentences()
    dataset1x1 = get_data_loaders_1sentence()
    if args.n_sentences == 4:
        dataset=dataset4x4
    elif args.n_sentences == 3:
        dataset = dataset3x3
    elif args.n_sentences == 2:
        dataset = dataset2x2
    else:
        dataset = dataset1x1
    ...
    if args.metric == 'all':
        ...
        a_file = open("Bleu_Metric3x3_separated_290522.json", "w")
        ...
        print(result)    
        a_file = open("Rouge_Metric3x3_separated_290522.json", "w")

        json.dump(result, a_file)

        a_file.close()     
        result = metric_cosine_similarity.compute(predictions=decoded_preds,references=decoded_labels,lang='en')  
        a_file = open("Cosine_Similarity_Metric3x3_separated_290522.json", "w")
