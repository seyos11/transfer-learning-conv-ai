...
def get_persona_faiss_selected(args):
    ...
    for dataset_name, dataset in personachat.items():
            ...
            for _ in range(args.personality_permutations):
                for utterance in dialog["utterances"]:
                    ...
                    for j, candidate in enumerate(utterance["candidates"][-num_candidates:]):
                        ...
                        persona_faiss_index.append([I[0][1:].tolist()])
                        persona_list = []
                        score_list = []
                        index_list = []
                        for i in I[0][1:]:
                        ...
                        #score_faiss.append(list(D.tolist()[0]))
        ...
        #break
    return persona_faiss_selected, persona_faiss_index, score_faiss
...
def train():
    ...
    parser.add_argument("--dataset_pkl", type=str, default="data_persona_faiss_fase1_opcion6_chatbot_all_mpnet_base.pkl", help="File where is saved the data from faiss (personalities)")
    args = parser.parse_args()
    persona_faiss, persona_faiss_index, score_faiss= get_persona_faiss_selected(args)
    with open(args.dataset_pkl, 'wb') as f:
        pickle.dump(persona_faiss, f)
    with open('data_persona_faiss_index_fase1_opcion6_chatbot_all_mpnet_base.pkl', 'wb') as f:
        pickle.dump(persona_faiss_index, f)
    with open('score_index_option6_all_mpnet_base.pkl', 'wb') as f:
        pickle.dump(persona_faiss_index, f)
