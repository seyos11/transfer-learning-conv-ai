            ...
            for _ in range(args.personality_permutations):
                for utterance in dialog["utterances"]:
                    ...
                    for j, candidate in enumerate(utterance["candidates"][-num_candidates:]):
                        ...
                        if len(history) > 1:
                            history_splitted_last_removed = " ".join(history[:-1])
                            history_encoded = model.encode([history_splitted_last_removed],show_progress_bar=False)
                        else:
...
def train():
    ...
    parser.add_argument("--dataset_pkl", type=str, default="data_persona_faiss_fase1_opcion7_chatbot_all_mpnet_base.pkl", help="File where is saved the data from faiss (personalities)")
