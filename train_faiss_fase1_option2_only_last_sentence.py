...
def get_persona_faiss_selected(args,tokenizer):
    ...
    for dataset_name, dataset in personachat.items():
        ...
        for dialog in dataset:
            ...
            embeddings_persona = model.encode(persona_complete, show_progress_bar=True)
            ...
            for _ in range(args.personality_permutations):
                for utterance in dialog["utterances"]:
                    ...
                    for j, candidate in enumerate(utterance["candidates"][-num_candidates:]):
                        history_encoded = model.encode(history,show_progress_bar=True)
...
def get_data_loaders(args, tokenizer):
    ...
    with open('data_faiss_fase1_opcion2.pkl', 'rb') as f:
